// 146. 베스트 앨범 (unordered_map, 구현) / re.. 풀었지만, 컨테이너 활용 더 잘하기 (2트 실패)
...
struct Song{
    int id, play;
    Song(int idIn, int playIn) : id(idIn), play(playIn) {}
};

vector<int> solution(vector<string> genres, vector<int> plays) {
    vector<int> ans;
    map<string, int> genrePlaySum;
    map<string, vector<Song>> genreSongs;
    
    for(int i=0; i<genres.size(); i++){
        genrePlaySum[genres[i]] += plays[i];
        genreSongs[genres[i]].emplace_back(i, plays[i]);    // emplace 쓰려면 매개변수 생성자 정의 필요 (push_back은 안해도 됨)
    ...
    for(const auto& [genre, sum] : sortedGenres){
        auto& songs = genreSongs[genre];    // auto : vector<Song>
